...
const CreateRepoPanel = (props) => {
    const [directorySelectTime, selectDirectory] = useState(0)
    const { input, getInput } = useHandleInput({})
    const tc = themeCollections[props.theme]

    const onDirectorySelected = async (files) => {
        setSessionStorage("files", Object.values(files).map(e => e.webkitRelativePath))

        selectDirectory(directorySelectTime + 1)
    ...
    return (
        <ColumnFlex bg={tc.background} br="0px" h="100%" p="16px" g="8px" style={{minWidth: "50%"}}>
            ...
            <Bar bcolor={tc.borderColor}>
                <Text cl={tc.text} fw="bold" text={`Create New Repo`} />
            </Bar>


            <div className={`file-input-wrapper ${props.theme == "dark" ? "dark " : "light"}`}>
                <button className={`btn-file-input ${props.theme == "dark" ? "btn-file-input-dark" : "btn-file-input-light"}`}>
                    Upload File</button>
                <input type="file" webkitdirectory="" multiple=""
                    onChange={e => onDirectorySelected(e.target.files)} />
            </div>
            ...
            <StyledButton bg={tc.primaryButton} hBg={tc.primaryButtonVariant} cl={tc.text}
                ...
                onClick={() => new RepoService().createRepo(input["repo"])} />
